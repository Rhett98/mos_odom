...
class feature_extraction(nn.Module):
    def __init__(self):
        ...
        self.branch1 = nn.Sequential(nn.AvgPool2d((16, 64), stride=(64,64)),
                                     convbn(128, 32, 1, 1, 0, 1),
                                     nn.ReLU(inplace=True))

        self.branch2 = nn.Sequential(nn.AvgPool2d((8, 32), stride=(32,32)),
                                     convbn(128, 32, 1, 1, 0, 1),
                                     nn.ReLU(inplace=True))

        self.branch3 = nn.Sequential(nn.AvgPool2d((4, 16), stride=(16,16)),
                                     convbn(128, 32, 1, 1, 0, 1),
                                     nn.ReLU(inplace=True))

        self.branch4 = nn.Sequential(nn.AvgPool2d((2, 8), stride=(8,8)),
                                     convbn(128, 32, 1, 1, 0, 1),
                                     nn.ReLU(inplace=True))
...
########
    ...
    def forward(self, left, right, tran_labels, rot_labels):
        ...
        if not self.training:
            with torch.no_grad():
                cost = Variable(torch.FloatTensor(refimg_fea.size()[0], refimg_fea.size()[1]*2, int(self.maxdisp/4),  refimg_fea.size()[2],  refimg_fea.size()[3]).zero_())
        else:
            cost = Variable(torch.FloatTensor(refimg_fea.size()[0], refimg_fea.size()[1]*2, int(self.maxdisp/4),  refimg_fea.size()[2],  refimg_fea.size()[3]).zero_())
        ...
        # torch.Size([1, 1, 48, 16, 512])
        cost = F.interpolate(cost, [self.maxdisp, 8, 256], mode='trilinear')
        ...
        tran = self.fully_connected_translation(cost)
        rot = self.fully_connected_rotation(cost)
        
        loss = {}
        loss_tran = self.l1_loss(tran_labels, tran)
        loss_rot =  self.l1_loss(rot_labels, rot)
        loss_sum = self.hws_loss(loss_tran, loss_rot)

        loss['tran'] = loss_tran
        loss['rot'] = loss_rot
        loss['sum'] = loss_sum
        return loss, tran, rot

    
if __name__ == '__main__':
    from thop import profile
    model = PSMNet(192)
    dummy_input = torch.randn(1, 5, 64, 2048),torch.randn(1, 5, 64, 2048),torch.randn(1, 3),torch.randn(1, 3),
